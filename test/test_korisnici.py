...
class KorisnikTest(unittest.TestCase):
    ...
    def test_kreiraj_validnog_korisnika(self):
        ...
        self.assertIsNotNone(svi_korisnici, msg="Nije vracena kolekcija korisnika")
    ...
    def test_azuriraj_validnog_korisnika(self):
        ...
        self.assertDictEqual(korisnik, svi_korisnici[korisnik["korisnicko_ime"]], msg="Korisnik nije dobro azuriran")
    ...
    def test_azuriraj_zauzeto_korisnicko_ime(self):
        ...
        with self.assertRaises(Exception, msg="Korisnicko ime je vec zauzeto: ocekuje se greska"):
    ...
    def test_email(self):
        ...
        with self.assertRaises(Exception, msg="Email provera sa @ ali sa vise poddomena"):
            korisnici.kreiraj_korisnika(
                {},
                False,  # azuriraj
                self.pun_korisnik["uloga"],
    ...
    def test_pasos(self):
        with self.assertRaises(Exception, msg="Pasos nebrojevni string"):
            korisnici.kreiraj_korisnika(
                {},
                False,  # azuriraj
                self.pun_korisnik["uloga"],
        ...
        with self.assertRaises(Exception, msg="Pasos manje od 9 cifara"):
            korisnici.kreiraj_korisnika(
                {},
                False,  # azuriraj
                self.pun_korisnik["uloga"],
        ...
        with self.assertRaises(Exception, msg="Pasos vise od 9 cifara"):
            korisnici.kreiraj_korisnika(
                {},
                False,  # azuriraj
                self.pun_korisnik["uloga"],
    ...
    def testiraj_korisnici_fajl(self):
        ...
        self.assertIsNotNone(ucitani_korisnici, msg="Nisu ucitani korisnici iz fajla")
        self.assertEqual(len(referentni_korisnici), len(ucitani_korisnici), msg="Duzine ucitanih korisnika nisu jednake")
        for korisnicko_ime in ucitani_korisnici:
            ucitani_korisnik = ucitani_korisnici[korisnicko_ime]
            self.assertDictEqual(referentni_korisnici[korisnicko_ime], ucitani_korisnik, msg="Ucitani korisnici se ne poklapaju")
    ...
    def testiraj_login(self):
        ...
        with self.assertRaises(Exception, msg="Login nepostojeci"):
            korisnici.login(svi_korisnici, "mika", "mikic")

        with self.assertRaises(Exception, msg="Login pogresna lozinka"):
            korisnici.login(svi_korisnici, korisnicko_ime, "321")
        pronadjeni_korisnik = korisnici.login(svi_korisnici, korisnicko_ime, lozinka)
        self.assertDictEqual(self.pun_korisnik, pronadjeni_korisnik, msg="Uspesan login")
